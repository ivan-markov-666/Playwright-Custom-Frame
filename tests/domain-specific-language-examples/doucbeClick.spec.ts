/**
 * @description     This class shows the solution for double-clicking mouse action over an element.
...
test.describe("This block contains examples for double-clicking mouse action over an element.", async () => {
  ...
  test("Playwright Example", async () => {
    ...
    let doubleClickButtonLocator: string = "#doubleClickBtn";
    // Call this method, to verify that the element is present and it is ready for usage.
    await dsl.element(doubleClickButtonLocator, 10000);
    // Make double-click mouse action over selected element.
    await page.dblclick(doubleClickButtonLocator, { force: true });
  });

  test("Domain-Specific Language Example", async () => {
    // Provide the locator.
    await dsl.doubleClick("#doubleClickBtn");
  });
});
