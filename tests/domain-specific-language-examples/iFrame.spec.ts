/**
 * @description     This class shows the solution for giving a focus to the iFrame.
...
test.describe("This block contains examples for giving a focus to the iFrame.", async () => {
  ...
  test.beforeEach(async () => {
    ...
    await dsl.navigateTo("https://demoqa.com/frames/");
  });

  //05. Create the "test" block/s.
  test("Playwright Example", async () => {
    // Declare an iFrame element. Provide the locator of the iFrame element.
    let iFrame = page.frameLocator("#frame1");
    // Declare an element. This element is positioned inside the iFrame.
    let iFrameElement = iFrame.locator("#sampleHeading");
    // Assert that the operation was compleated correctly.
    await dsl.getText(iFrameElement, "This is a sample page");
  });
