/**
 * @description     This class shows the solution for accepting alert pop-up windows.
...
test.describe("This block contains examples for accepting alert pop-up windows.", async () => {
  ...
  test("Playwright Example", async () => {
    ...
    page.once("dialog", async (dialog) => {
      // Аssert to verify that the pop-up window contains the expected text.
    ...
    expect(await dsl.getText("#confirmResult")).toEqual("You selected Ok");
  });
  ...
  test("Domain-Specific Language Example 1", async () => {
    ...
    dsl.alertAccept("#confirmButton", "Do you confirm action?");
    // Assert that the operation was compleated correctly.
    await dsl.getText("#confirmResult", "You selected Ok");
  });

  test("Domain-Specific Language Example 2", async () => {
    // Provide the locator of a button that triggers the alert pop-up window.
    dsl.alertAccept("#confirmButton");
    // Assert that the operation was compleated correctly.
    await dsl.getText("#confirmResult", "You selected Ok");
  });
});
