/**
 * @description     This class shows the solution for checking radio buttons or checkboxes.
...
test.describe("This block contains examples for checking radio buttons or checkboxes.", async () => {
  ...
  test("Playwright Example 1", async () => {
    ...
    let checkBoxLocator: string = "//label[@for='tree-node-home']";
    // Create the element.
    let checkBoxElement = await dsl.element(checkBoxLocator, 10000);
    // Verify the element is not checked.
    expect(await page.isChecked(checkBoxLocator)).toBeFalsy();
    await expect(checkBoxElement).not.toBeChecked();
    // Check the element using "check" action.
    await page.check(checkBoxLocator, { force: true });
  ...
  test("Playwright Example 2", async () => {
    ...
    await page.click(checkBoxLocator, { force: true });
    // Verify the element is checked.
    await expect(checkBoxElement).toBeChecked();
    expect(await page.isChecked(checkBoxLocator)).toBeTruthy();
  });
  ...
  test("Domain-Specific Language Example 1", async () => {
    ...
    await dsl.checkRadioButtonCheckBox("//label[@for='tree-node-home']");
  });
