/**
 * @description     This class shows the solution for getting a text from an element that contains the text.
...
test.describe("This block contains examples for getting a text from an element that contains the text.", async () => {
  ...
  test("Playwright Example", async () => {
    ...
    let element = await dsl.element("#userName-label", 10000);
    ...
    expect(elementTextValue).toEqual("Full Name");
  });

  test("Domain-Specific Language Example 1", async () => {
    // Provide the locator and expected text value.
    await dsl.getInnerText("#userName-label", "Full Name");
  });

  test("Domain-Specific Language Example 2", async () => {
    // Provide the locator
    let textValue = await dsl.getInnerText("#userName-label");
    // Assert that the operation was compleated correctly.
    expect(textValue).toEqual("Full Name");
  });
  ...
  test("Domain-Specific Language Example 3", async () => {
    ...
    await dsl.getInnerText(element, "Full Name");
  });
  ...
  test("Domain-Specific Language Example 4", async () => {
    ...
    let element = await dsl.element("#userName-label", 10000);
    // Provide the element.
    let textValue = await dsl.getInnerText(element);
    // Assert that the operation was compleated correctly.
    expect(textValue).toEqual("Full Name");
  });
});
