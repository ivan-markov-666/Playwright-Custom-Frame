/**
 * @description     This class shows the solution for hovering over the element.
...
test.describe("This block contains examples for hovering over the element.", async () => {
  ...
  test.beforeEach(async () => {
    ...
    await dsl.navigateTo("https://demoqa.com/tool-tips/");
  });

  //05. Create the "test" block/s.
  test("Playwright Example", async () => {
    // Declare a locator of an element.
    let clickElementLocator: string = "#toolTipButton";
    ...
    await page.hover(clickElementLocator, { force: true });
  });

  test("Domain-Specific Language Example", async () => {
    //  Provide the locator.
    await dsl.hover("#toolTipButton");
  });
});
