/**
 * @description     This class shows the solution for right-clicking mouse action over an element.
...
test.describe("This block contains examples for right-clicking mouse action over an element.", async () => {
  ...
  test("Playwright Example", async () => {
    ...
    let rightClickButtonLocator: string = "#rightClickBtn";
    // Call this method, to verify that the element is present and it is ready for usage.
    await dsl.element(rightClickButtonLocator, 10000);
    // Make right-click mouse action over selected element.
    await page.click(rightClickButtonLocator, { button: "right", force: true });
  });

  test("Domain-Specific Language Example", async () => {
    // Provide the locator.
    await dsl.rightClick("#rightClickBtn");
  });
});
